...
describe("Optimize", function() {
  ...
  describe("Hoisting", function() {
    ...
    it("merges Where expressions to NamedRelation", function() {
      ...
      expect(namedS.predicates[0].tree()).to.deep.equal("$$cmp(s.id, 2) <= 0 && $$cmp(s.id, 1) >= 0");
    })
    ...
    it("merges Where predicates that depend on right side of outer Join", function() {
      // The predicate might depend on whether a tuple is generated by the join.
      let join = new Join(namedS, namedT, "outer");

      let termGroups1 = new TermGroups();
      termGroups1.parse("t.id === undefined", {t: namedT});
      let where1 = new Where(join, termGroups1);

      let termGroups2 = new TermGroups();
      termGroups2.parse("t.otherId === undefined", {t: namedT});
      let where2 = new Where(where1, termGroups2);
      
      expect(hoistPredicates(where2)).to.equal(join);
      expect(namedS.predicates).to.deep.equal([]);
      expect(namedT.predicates).to.deep.equal([]);
      expect(join.predicates.map(p => p.tree())).to.deep.equal(["t.otherId === undefined && t.id === undefined"]);
    })
